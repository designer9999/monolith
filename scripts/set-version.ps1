...
function Get-AndroidVersionCode([string]$Version) {
  ...
  if ($parts.Count -ne 3) {
    throw "Android versionCode requires x.y.z SemVer. Got: $Version"
  }
...
try {
  ...
  $androidPropsPath = Join-Path $Root "src-tauri\gen\android\app\tauri.properties"
  if (Test-Path $androidPropsPath) {
    $versionCode = Get-AndroidVersionCode $Version
    $androidProps = @(
      "# THIS FILE IS AUTOMATICALLY GENERATED BY TAURI",
      "# DO NOT EDIT",
      "",
      "tauri.android.versionName=$Version",
      "tauri.android.versionCode=$versionCode"
    ) -join "`n"
    Write-Utf8NoBom $androidPropsPath ($androidProps + "`n")
  }
